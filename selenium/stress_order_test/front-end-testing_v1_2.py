...
is_headless = os.environ.get('HEADLESS')
...
class hekayati_success_scanario:
    ...
    def get_home(self):
        ...
        self.product_url = [
            f"{base_url}/story/football-star/",
            f"{base_url}/story/the-falcon/",
            f"{base_url}/story/space-mission/"
        ]                
        pass
    ...
    def get_preview(self):
        ...
        story_first_name = os.environ.get('STORY_FIRST_NAME') 
        driver.find_element(By.NAME, "first_name").send_keys(story_first_name)
    ...
    #     driver.find_element_by_css_selector("#billing_first_name").clear().send_keys(billing_first_name)
...
driver.get( f"{base_url}/en/checkout/" )
# ### Placing Order

try:
    billing_first_name  = os.environ.get('BILLING_FIRST_NAME')
    driver.find_element_by_css_selector("#billing_first_name").send_keys(billing_first_name)
except Exception as e:
...
WebDriverWait(driver, 200).until(EC.url_contains(f"{base_url}/en/checkout/order-received"))


# In[ ]:


print("ORDER SUCCESSFULLY PLACES...")
